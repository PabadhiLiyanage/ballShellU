...
func main() {
	...
	if len(os.Args) > 1 {
		...
		if runCommand {
			if os.Args[1] == "build" {
				...
				cmd := exec.Command(javaCommand, "-jar", javaCommandPath, "build")
				...
				if runtime.GOOS == "darwin" {
					//set an environmental variable for architecture in mac
					os.Setenv("BALLERINA_MAC_ARCHITECTURE", runtime.GOARCH)
				}
				javaCommandPath := filepath.Join(scriptDir, "..", "lib", "ballerina-command-1.4.2.jar") //ballerina-command-@version@.jar
				cmd := exec.Command(javaCommand, append([]string{"-jar", javaCommandPath}, os.Args[1:]...)...)
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr
				err := cmd.Run()
				if err != nil {
					fmt.Println("Failed to execute Ballerina Command JAR:", err)
			...
			if os.Args[1] == "update" {
				...
				if _, err := os.Stat(tmpDir); err == nil {
					...
					fmt.Println("There is no file path such that ", tmpDir)
					os.Exit(1)
				}
			}
		}
	...
	if runBallerina {
		...
		fmt.Println("user version", userBallerinaVersion)
		if err != nil {
		...
		fmt.Println("Ballerina home ", ballerinaHome)
		...
		balPath = "/usr/lib/ballerina/distributions/ballerina-2201.8.4/bin/ball"
